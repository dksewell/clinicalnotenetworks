#' Compute within/between communication measures disregarding setting
#' 
#' @details
...
within_between_no_setting = function(id_pat,
                                     db,
  ...
  data_involvement = 
    ...
    filter(mts != "Exclude",
           mts != "Scribe",
           mts != "Medical Student")  # Just a check in case someone changes code above - "Ha! I just did, pastDan."  -currentDan
  ...
  unique_mts = 
    unique(mts$mts) |> 
    setdiff(c("Scribe","Exclude","Medical Student"))
      ...
      if(length(applicable_teams_other) > 0){
        ...
        for(k in applicable_teams_other){
          ...
          if(nrow(from_team_k) > 0){
            ...
            network_measures[[paste("between_reach",
                                    paste0(k,":",j),
                                    sep="_")]][wk] = 
              actual_reads
            #### Compute 7. btwn team share
            network_measures[[paste("between_share",
                                    paste0(k,":",j),
                                    sep="_")]][wk] = 
              actual_reads / 
              potential_reads
            ### Compute 8a. w/in team missed opportunities 
            network_measures[[paste("between_missedoppsA",
                                    paste0(k,":",j),
                                    sep="_")]][wk] = 
              potential_reads - actual_reads
            
            
            ### Compute 8b. w/in team missed opportunities 
            network_measures[[paste("between_missedoppsB",
                                    paste0(k,":",j),
                                    sep="_")]][wk] = 
              data_edges |> 
              #### Filter to only notes written by team j
